...
|---|---|---|---|
| "Is this rule-set usable?" | `srs_reject_reason(head, size)` | `bin/sc` `# Rule-sets` | The single definition (SRS magic + size floor). Three adapters: `ruleset_status(path)` for a file, `_fetch_to_temp()` for bytes off a socket, `_status_text()` for the screen. Never form a second opinion. |
| Per-file rule-set state | `ruleset_report()` → `[(tag, filename, status)]`, `usable_tags(report)` | same | Pure query: no network, no service, no config, writes nothing. `status` is a flat `str`: `usable / absent / bad-magic / too-small / unreadable`. |
