...
| id | row | owner |
|---|---|---|
| R-62 | **T-23's encoding defect reaches `nodes.json` and `config.json`, not only the state file — and this task widened who can hit it.** Measured at T-22 stage 6 under `LC_ALL=C PYTHONUTF8=0`: `trojan://p%C3%A9q@h.example:443` raises `UnicodeEncodeError: 'ascii' codec can't encode character '\xe9'` out of `sc`'s own writers (`bin/sc:477` `_write_private`, `:549` `save_nodes`, `:2079` `generate_config`). **Identical at HEAD, so not a regression** — but T-22 makes non-ASCII credentials *reachable* for tuic / trojan / hysteria2 shapes whose passwords used to be emptied or truncated before they ever reached a writer, so the population that can trip it grew. T-23's row is scoped to `settings.json`/`nodes.json` *reads*; this is the **write** side and the credential document. QA-1, MINOR. | **T-23** `state-file-io-contract` |
