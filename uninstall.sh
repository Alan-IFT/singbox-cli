...
t() {
    ...
    if [ "$LANG_CHOICE" = "zh" ]; then
        case "$key" in
            ...
            cancelled)      fmt="已取消" ;;
            purge_q)        fmt="是否同时移除 sing-box 二进制和 APT 源？这会让本机失去 sing-box 内核 [y/N] " ;;
            purge_done)     fmt="  已移除 sing-box 与 APT 源" ;;
        ...
        case "$key" in
            ...
            purge_q)        fmt="Also remove the sing-box binary and APT source? This leaves the machine without the sing-box core. [y/N] " ;;
            purge_done)     fmt="  Removed sing-box and APT source" ;;
